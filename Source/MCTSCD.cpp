...
{
	GameNode* newNode = new GameNode(parent, gs);
...
void MCTSCD::deleteAllChildren(GameNode* node)
{
	for (auto& child : node->children) deleteAllChildren(child);
	delete node;
}

MCTSCD::GameNode* MCTSCD::bestChild(GameNode* currentNode)
{
	...
	if (currentNode->playerTurn == -1) return currentNode;

	// if first time here return this node
	if (currentNode->totalVisits == 0) return currentNode;

	// Bandit policy (aka Tree policy)
	// 	GameNode* best = UCB(currentNode);
	// 	GameNode* best = eGreedy(currentNode);
	// 	GameNode* best = eGreedyInformed(currentNode);
	GameNode* best = PUCB(currentNode);
...
MCTSCD::GameNode* MCTSCD::createChild(GameNode* node, playerActions_t action)
{
	if (!action.empty()) {
		node->actions.push_back(action);
		GameState gs2 = node->gs.cloneIssue(action, node->moveGenerator._player);
		GameNode* newChild = newGameNode(gs2, node);
...
void MCTSCD::printNodeError(std::string errorMsg, GameNode* node)
{
	DEBUG(errorMsg);
	LOG(node->gs.toString());
	LOG("playerTurn: " << node->playerTurn);
}

MCTSCD::GameNode* MCTSCD::eGreedyInformed(GameNode* currentNode)
{
	...
	if (randomNumber < EPSILON) { // select bias random
		...
		for (const auto& child : currentNode->children) {
			...
			if (currentNode->playerTurn == 0) tmpScore = -tmpScore; // if min node, reverse score
...
MCTSCD::GameNode* MCTSCD::eGreedy(GameNode* currentNode)
{
	// if no children yet, create one
	if (currentNode->children.empty()) {
		if (!currentNode->moveGenerator.hasMoreActions()) {
...
MCTSCD::GameNode* MCTSCD::UCB(GameNode* currentNode)
{
...
double MCTSCD::nodeValue(MCTSCD::GameNode* node)
{
...
MCTSCD::GameNode* MCTSCD::PUCB(GameNode* currentNode)
{
	...
	if (currentNode->children.empty()) {
		...
		while (currentNode->moveGenerator.hasMoreActions()) {
			...
			GameNode* child = createChild(currentNode, action);
			child->prob = prob;
		}
