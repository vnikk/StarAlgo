...
private:
    ...
    double                getActionProbability(action_t action, const std::map<probName, uint8_t>& possibleActions,
                                               unitGroup_t* currentGroup);
    double                getProbabilityGivenState(probName action, const std::map<probName, uint8_t>& possibleActions);
    bool                  isMovingTowardsBase(const std::vector<unitGroup_t*>* groupList, uint8_t currentRegion,
                                              uint8_t targetRegion);
